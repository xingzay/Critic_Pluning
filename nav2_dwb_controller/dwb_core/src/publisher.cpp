...
DWBPublisher::on_configure()
{
  ...
  declare_parameter_if_not_declared(
    node, plugin_name_ + ".my_publish_local_plan",
    rclcpp::ParameterValue(true));
  ...
  node->get_parameter(plugin_name_ + ".my_publish_local_plan", my_publish_local_plan_);
  ...
  local_pub_my = node->create_publisher<nav_msgs::msg::Path>("my_local_plan", 1);
...
DWBPublisher::publishLocalPlan(
  ...
  bool my_plan)
{
  if (!publish_local_plan_&&!my_publish_local_plan_) {return;}
  ...
  if (!my_plan&&local_pub_->get_subscription_count() > 0) {
    local_pub_->publish(std::move(path));
  }else if (my_plan && local_pub_my->get_subscription_count() > 0) {
   local_pub_my->publish(std::move(path));
  }
...
void
DWBPublisher::my_publishLocalPlan(const nav_2d_msgs::msg::Path2D plan)
{
  publishGenericPlan(plan, *local_pub_my, my_publish_local_plan_);
}
